...
- This is a local Git repo tracked with GitHub Desktop — the user pushes/syncs manually, so never run `git push`.
...
- Do not: run `git push` (see Workflow above — user syncs manually via GitHub Desktop).
